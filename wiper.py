...
if __name__ == "__main__":
    hil = WiperWasherHILSimulator(com_port="COM6")
    try:
        hil.connect()

        # Turn ON wiper for 5 seconds
        hil.set_input_signal("Set_Wiper_ON", 1, duration=5)
        response = hil.get_output_signal("Wiper_Status")
        print(f"Wiper_Status: {response}")

        # Turn ON washer for 3 seconds
        hil.set_input_signal("Set_Washer_ON", 1, duration=3)
        response = hil.get_output_signal("Washer_Status")
        print(f"Washer_Status: {response}")

        # Receive some raw data (optional)
